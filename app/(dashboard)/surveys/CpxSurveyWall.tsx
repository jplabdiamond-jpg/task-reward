...
interface Props {
  ...
  appId: string       // CPX APP ID (env経由でSSRから渡す)
}
...
export default function CpxSurveyWall({ userId, appId }: Props) {
  ...
  useEffect(() => {
    if (!userId || !appId || !containerRef.current) return
    ...
    script.src = `https://t.cpx-research.com/app/api/script-tag/v2/survey-widget.js?app_id=${appId}&ext_user_id=${encodeURIComponent(userId)}&output_method=inline&survey_distributor=random&widget_type=wall`
    script.async = true
    script.setAttribute('data-cpx-app-id', appId)
