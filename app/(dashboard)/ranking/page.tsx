...
export default async function RankingPage() {
  ...
  const RankIcon = ({ pos }: { pos: number }) => {
    ...
    return <span className="text-xs text-[#6b7280] font-bold w-4 text-center">{pos}</span>
  }
    ...
    <div className="space-y-4 pb-20 md:pb-0">
      ...
      {myRank > 0 && (
        <div className="card p-4 border-l-4 border-green-500">
          <div className="text-xs text-[#b8bcc8] mb-1">あなたの順位</div>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-2xl font-bold text-green-400">{myRank}位</span>
              <span className="text-sm text-[#b8bcc8]">/ {allUsers?.length ?? 0}人中</span>
            </div>
            {myRank > 3 && (
      ...
      {(allUsers ?? []).length >= 3 && (
        <div className="flex items-end gap-2 justify-center py-2">
          ...
          <div className="flex-1 card p-3 text-center">
            ...
            <div className="text-xs text-[#b8bcc8]">{formatCurrency(allUsers![1].total_earned)}</div>
          </div>
          {/* 1位 */}
          <div className="flex-1 card p-3 text-center bg-gradient-to-b from-yellow-50 to-white border-yellow-200">
            ...
            <div className="text-sm font-bold text-green-400">{formatCurrency(allUsers![0].total_earned)}</div>
          </div>
          {/* 3位 */}
          <div className="flex-1 card p-3 text-center">
            ...
            <div className="text-xs text-[#b8bcc8]">{formatCurrency(allUsers![2].total_earned)}</div>
          </div>
        </div>
      )}
      ...
      <div className="card divide-y divide-[rgba(0,0,0,0.06)]">
        {(allUsers ?? []).map((u, i) => (
          <div key={u.id}
            className={`flex items-center gap-3 px-4 py-3 ${u.id === user!.id ? 'bg-bg-blue-500/10' : ''}`}>
            <div className="w-5 flex-shrink-0 flex justify-center">
              <RankIcon pos={i + 1} />
            </div>
            <div className="w-8 h-8 rounded-full bg-[#0e1014] flex items-center justify-center text-sm font-bold flex-shrink-0"
